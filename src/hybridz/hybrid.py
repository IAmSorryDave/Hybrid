...
def hybrid(node):
    ...
    def wrapper(__class_or_instance=None, /, *args, **kwargs):
        nonlocal node
        copy_of_node = FunctionType(
            node.__code__,
            node.__globals__,
            name=node.__name__,
            argdefs=node.__defaults__,
            closure=node.__closure__,
        )
        copy_of_node.__kwdefaults__ = (
            node.__kwdefaults__.copy() if node.__kwdefaults__ is not None else None
        )
            ...
            for key in dir(__class_or_instance):
                if key in param_names:
                    ...
                    if (
                        ...
                        and attribute.__code__ == node.__code__
                    ):
                        copy_of_node = node
                        break
